...
For example, **Fix job** includes all failed steps and their error logs.
**Fix workflow** includes the workflow ID, the list of failed jobs, and the execution sequence.
...
The setup flow works as follows:

. Click any fix button on a failed build, job, workflow, or step.
. The Chunk intro modal appears.
  Select **Get Started**.
. If you are an organization admin, follow the on-screen steps to install the CircleCI GitHub App and enter your AI model provider API key.
...
If you are not an organization admin, the following happens instead:

. Select **Notify my admin** in the modal.
  An email is sent to all organization admins requesting Chunk access.
. A confirmation message appears to confirm the notification was sent.
  Your admin will need to complete the setup before you can use Chunk.
